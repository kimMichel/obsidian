...
- Use indexes rather than actual portions of file names when templating or using language files (e.g. value 5 from the user submission = Czechoslovakian, rather than expecting the user to return "Czechoslovakian").
...
See the [OWASP Testing Guide](https://owasp.org/www-project-web-security-testing-guide/)article.
...
Will result in the Java application seeing a string that ends with ".pdf" and the operating system will see a file that ends in ".doc". Attackers may use this trick to bypass validations routines.

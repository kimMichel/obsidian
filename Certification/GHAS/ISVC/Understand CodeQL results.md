...
Information also includes how to fix the problem when CodeQL analysis identifies an alert. Additionally, code scanning through CodeQL can detected data-flow problems in your code.

![[Pasted image 20251212113603.png]]

# Data-flow alerts

Data-flow analysis find potential security issues in code, including:
...
Dismissing an alert is a way of closing an alert that you don't think needs to be fixed. For example, you might dismiss an alert for an error in code used only for testing. You might also dismiss an alert if the effort required to fix the error is greater that the potential benefit of improving the code.

You can dismiss alerts from code-scanning annotations in the code or from the summary list on the Security tab. To dismiss an alert from the list, select a reason for dismissal, and then select the Dismiss alert button.
...
- The alert appears again in your analysis results, if the code that generated the alert stays the same, and the same code-scanning tools runs again without any configuration changes.

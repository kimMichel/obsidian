...
We recommend that you install the CodeQL CLI and queries by downloading the bundled package. This method helps ensure compatibility and improve performance, as opposed to downloading the CLI and queries separately.
...
After you successfully create the database, a new directory appears at the path specified in the command. If you used the ``--db-cluester`` option to create more than one database, a subdirectory is created for each language.
...
- The ``statement`` table contains a row for every statement in the source code that CodeQL analyzed during the build process.
...
You can use Visual Studio Code and the CodeQL extension to compile and run queries, as long as you're using VS Code 1.39 or later.

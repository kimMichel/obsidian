...
Metadata information can include a description of the query, a unique ID, and the kind of problems it is (alert or path). Metadata also specifies how to interpret and display the query results.
...
![[Pasted image 20251208104057.png]]

CodeQL doesn't interpret queries that you don't have metadata. It shows those results as a table and doesn't display them in the source code.
...
Your path query requires certain metadata, query predicates, and ``select`` statement structures. Many of the built-in path queries in the CodeQL follow a basic structure. The structure depends on how CodeQL models the language that you're analyzing.
...
``import semle.code.cpp.ir.dataflow.internal.DataFlowImpl``
...
``codeql database analyze --format=<foramt> --output=<output> [--threads=<num>] [--ram=<MB>] <options>... --<dabase> <query|dir|suite>...``
...
CodeQL supports SARIF for sharing static analysis results. SARIF is designed to represent the output a broad range of static analysis tools.
...
``code ql github upload-results --sarif=<file> [--github-auth-stdin] [--github-url=<url>] [--repository=<repository-name>] [--ref=<ref>] [--commit=<commit>] [--checkout-path=<path>] <options>...``
...
Each time you upload the results of a new code scan, CodeQL processes the results and adds alerts to the repository. To prevent duplicate alerts for the same problem, code scanning uses the SARIF ``partialFingerprints`` property to match results across various runs so that they appear only once in the latest run for the select branch. Eliminating duplicates make it possible to match alerts to the correct line of code when files are edited.

...
- Dependency graph is automatically updated when you push a commit to Github changes or adds supported manifest or lock file to the default branch. It's also automatically updated when anyone pushes a change to the repository of one of your dependencies as well.
...
Ignoring a security alert poses significant risks to the project. Vulnerabilities may be exploited by malicious actors, leading to data breaches, services disruptions, or other security incidents. Ignoring alerts can also result in increased remediation efforts, potentially impacting projects timelines and the overall trustworthiness of the software.
